...
def agent_node(agent_name: str, agent_skill: str):
    ...
    def node(state: TeamState) -> TeamState:
        ...
        if decision == "执行" and action != "无" and action != "「无」":
            ...
            # 修改：(可选：可以使用原来的，我这是有点儿强迫症，所以给改了) 命中片段占总片段数 >= 50%才可以移除
            def _is_done(t, act, window=4, threshold=0.5):
                windows = [t[i:i+window] for i in range(max(1, len(t) - window + 1))]
                return sum(1 for w in windows if w in act) / len(windows) >= threshold
            new_state["todo_tasks"] = [t for t in new_state["todo_tasks"] if not _is_done(t, action)]
            
            # ③ 添加状态更新记录：让其他智能体感知「该智能体完成了什么」，实现团队信息同步
            new_state["status_updates"] = list(new_state["status_updates"]) + [f"{agent_name}：{action}"]

            # 新增：如果todo_tasks全部完成时自动设为True
            new_state["is_finished"] = len(new_state["todo_tasks"]) == 0
