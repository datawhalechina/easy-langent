...
> 本案例使用deepseek作为学习演示的底座模型，也可以使用其他的底座模型，对后面的应用没有任何影响，只要模型是兼容openai的接口即可，几乎常见的大模型都支持。
...
> 这里了解即可，无需大家去实践~
举个例子：我们可以去hugging face的镜像站hf-mirror下载一个小模型Qwen2.5-0.5B[点击跳转](https://hf-mirror.com/Qwen/Qwen2.5-0.5B-Instruct/tree/main)  
点击`Files and versions`下载对应的文件，举个例子，这个模型需要这些下列的json和model文件。最好txt文件也一并下载。
```
这是项目根目录
├── 你的代码.py
├── models
...
model_name = "./models/qwen2.5_0.5"  # 模型名 这是本地路径 这里替换成自己的路径！！
...
pipe = pipeline(
    "text-generation",
    model=model, # 选择对话模型
    tokenizer=tokenizer, # 选择模型分词器(如果模型有tokenizer.json文件一般需要他)
    max_new_tokens=200, # 最大生成 tokens 数，避免生成过长内容
    temperature=0.3  # 随机性：0-1，越小越严谨，越大越有创造力generation_config.json文件里面有时候可能也会定义temperature，代码优先级更高。
)
