...
from langchain_core.tools import tool  # 导入自定义工具的装饰器
from langchain_openai import ChatOpenAI  # 补充LLM的导入
import os
from dotenv import load_dotenv
...
# 1. 自定义天气查询工具（如有条件可使用外部api）
# 使用@tool装饰器是LangChain定义自定义工具的标准方式
@tool
def weather_query(city: str) -> str:
    """
    用于查询指定城市的实时天气、温度、降水概率等信息（教学演示用，返回固定数据）
    参数：
        city: 要查询的城市名称（字符串）
    返回：
        该城市的模拟天气信息
    ...
    # 模拟固定的天气数据
    weather_data = {
        "北京": "北京今日天气：晴，温度-2~8℃，北风2级，降水概率0%，空气质量优",
        "上海": "上海今日天气：多云，温度5~12℃，东风1级，降水概率10%，空气质量良",
        "广州": "广州今日天气：阴转小雨，温度18~25℃，南风3级，降水概率60%，空气质量良",
        "深圳": "深圳今日天气：雷阵雨，温度20~28℃，东南风4级，降水概率80%，空气质量优",
    }
    # 如果查询的城市不在预设列表中，返回默认提示
    return weather_data.get(city, f"暂未查询到{city}的天气数据（教学演示版）")
...
prompt = ChatPromptTemplate.from_messages([
    ("system", "你是一名信息查询助手，只能使用提供的工具查询信息。如果不需要工具，直接回答。"),
    ("user", "{input}"),
    ("assistant", "{agent_scratchpad}")  # 留给AI记录思考和工具调用过程的地方
])

# 4. 构建Agent和执行器
agent = create_openai_tools_agent(llm, tools, prompt)
# verbose=True：显示详细的思考和执行过程，适合学习演示
agent_executor = AgentExecutor(agent=agent, tools=tools, verbose=True)

# 5. 执行查询（测试不同城市）
if __name__ == "__main__":
    # 测试1：查询预设城市（北京）
    result1 = agent_executor.invoke({"input": "查询北京今天的实时天气"})
    print(f"\n最终结果1：{result1['output']}\n")
    
    # 测试2：查询另一个预设城市（上海）
    result2 = agent_executor.invoke({"input": "上海今天温度多少？"})
    print(f"最终结果2：{result2['output']}\n")
    
    # 测试3：查询未预设的城市（成都）
    result3 = agent_executor.invoke({"input": "成都今天的天气怎么样？"})
    print(f"最终结果3：{result3['output']}")
```
...
responded: 我来帮您查询北京今天的实时天气。

北京今日天气：晴，温度-2~8℃，北风2级，降水概率0%，空气质量优根据查询结果，北京今天的实时天气情况如下：
...
今天北京天气晴朗，气温在零下2度到8度之间，没有降水，空气质量很好，是个不错的天气！

> Finished chain.

最终结果1：根据查询结果，北京今天的实时天气情况如下：
...
# 2. 自定义温度转换工具（修复@tool装饰器参数问题）
@tool(args_schema=TemperatureConvertInput)  # 新版仅保留args_schema参数（如需参数校验）
def temperature_converter(temperature: float, from_unit: str) -> str:
    """
    用于温度单位转换的工具，支持摄氏度（celsius）与华氏度（fahrenheit）双向互转。
    
    参数说明：
        temperature: 数值型温度值（如37.0）
        from_unit: 原始单位，仅支持'celsius'或'fahrenheit'
    """
    # 校验单位合法性
    if from_unit not in ["celsius", "fahrenheit"]:
        return f"错误：单位'{from_unit}'不合法，仅支持'celsius'（摄氏度）或'fahrenheit'（华氏度）"
...
# 3. 配置Agent（关键：Prompt必须包含MessagesPlaceholder用于agent_scratchpad）
tools = [temperature_converter]

# 正确的Prompt模板
prompt = ChatPromptTemplate.from_messages([
    ("system", "你是一名专业的温度转换助手，仅使用提供的TemperatureConverter工具完成转换任务，严格按照工具要求传参。"),
    ("human", "{input}"),
    MessagesPlaceholder(variable_name="agent_scratchpad"),  # 必须保留，用于Agent思考过程
])
...
# 执行转换任务
if __name__ == "__main__":
    result = agent_executor.invoke({"input": "将37摄氏度转换为华氏度"})
    print(f"\n最终结果：{result['output']}")
```
...
responded: 我来帮您将37摄氏度转换为华氏度。

37.0摄氏度 = 98.60华氏度37摄氏度等于98.60华氏度。

> Finished chain.

最终结果：37摄氏度等于98.60华氏度。
```
...
除了自定义工具意外，LangChain还有很多内置工具，不用大家重新去设计，可以直接使用，非常方便。

**例如文件操作类工具**

- ReadFileTool：读取本地文件的内容（比如读取txt、docx文件）；
- WriteFileTool：把内容写入本地文件（比如生成报告后保存到电脑）；
- ListDirectoryTool：查看指定文件夹里有哪些文件。
...
# 1. 导入LangChain内置的文件写入工具
from langchain.tools.file_management.write import WriteFileTool

# 2. 定义要使用的工具列表（此处使用WriteFileTool）
tools = [WriteFileTool()]

# 3. 构建Prompt模板，明确告知Agent任务：创建llm诗词.txt并写入一首诗
prompt = ChatPromptTemplate.from_messages([
    ("system", "你是一个擅长使用工具完成文件操作的助手，严格按照工具要求执行任务。"),
    ("user", "请你创建一个名为llm诗词.txt的文件，并在文件中写入一首原创的七言绝句，主题围绕科技与人文的融合。"),
    MessagesPlaceholder(variable_name="agent_scratchpad"),  # 用于Agent记录思考过程
])

# 4. 创建Agent（将LLM、Prompt、工具绑定）
agent = create_openai_tools_agent(llm, tools, prompt)

# 5. 创建Agent执行器（负责调度Agent执行任务）
agent_executor = AgentExecutor(agent=agent, tools=tools, verbose=True)

# 6. 运行Agent，执行创建文件并写入诗词的任务
result = agent_executor.invoke({"input": ""})

# 7. 任务完成后，输出提示信息
print("任务执行完成！已创建llm诗词.txt并写入诗词。")
```

运行结果

```
任务执行完成！已创建llm诗词.txt并写入诗词。
《智融古今》
...
chain = (
    RunnablePassthrough.assign(
        chat_history=lambda x: x["chat_history"]  # 传递窗口记忆
    )
    | RunnableLambda(judge_and_calc)  # 注入计算工具调用逻辑
    | prompt  # 拼接“窗口记忆+增强输入”
    | llm  # 调用LLM生成最终回答
)
