...
| Word（.docx）   | DocxLoader                                                 | `pip install python-docx`（无需旧版 docx2txt）               |
...
官方已淘汰旧版 Docx2txtLoader，当前推荐 DocxLoader（langchain-community 内置），支持读取Word中的文本、表格等内容，稳定性更强，无需依赖第三方 docx2txt 库。
...
# 加载文档（无需安装docx2txt，仅需提前安装python-docx）
