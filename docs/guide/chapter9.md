...
> **提示：** LangChain 本身不内置 Agent Skills 功能，但 DeepAgents 框架（由 LangChain 团队开发）提供了对 Skills 的原生支持，可无缝集成并便捷调用 Agent Skills。DeepAgents 框架怎么和lang框架搭配使用将在后面的章节进行介绍。
>
> 本节是站在langchain的角度去讲解怎么实现Skills相关的功能设计~
...
@tool
def initialize_analysis(dataset_name: str, runtime: ToolRuntime) -> Command:
    """初始化数据分析会话"""
    # 更新状态，保存数据集信息
    return Command(
        update={
...
@tool
def perform_analysis(runtime: ToolRuntime) -> str:
    """执行分析，使用状态中的数据集信息"""
    dataset = runtime.state.get("current_dataset", "未知数据集")
    return f"正在分析数据集: {dataset}"
```
