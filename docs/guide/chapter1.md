...
什么是LangChain？
LangChain 是构建完全自定义、由大语言模型（LLM）驱动的智能体（Agent）和应用的最简便方式。只需不到 10 行代码，你就能连接到 OpenAI、Anthropic、Google 等模型。LangChain 提供了预构建的智能体架构和模型集成方案，旨在帮助你快速上手，并将 LLM 无缝融入到你的智能体与应用之中。
